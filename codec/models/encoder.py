...
class EncoderStage1D(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, *, train: bool = False, rng: Any | None = None) -> jnp.ndarray:
        del rng
        h = x
...
class SimVQEncoder1D(nn.Module):
    ...
    def setup(self) -> None:
        ...
        stages = []
        for idx, (stride, stage_blocks) in enumerate(zip(self.down_strides, stage_res_blocks)):
            stages.append(
                EncoderStage1D(
                    in_ch=channels[idx],
