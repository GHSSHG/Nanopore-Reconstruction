...
from .transformer import LocalTransformerBlock1D

_DEFAULT_DORADO_FEATURE_LAYERS = ("conv4", "conv5", "tx6", "tx12", "tx18", "upsample")
...
def _normalize_layer_name(name: str) -> str:
    ...
    if raw in {"upsample", "crf_probs", "crf_logits"}:
        return raw
    if raw.startswith("conv"):
        ...
        raise ValueError(f"Unsupported Dorado layer tap name: {name!r}")
    suffix = raw[len(prefix) :]
    try:
        layer_num = int(suffix)
    except ValueError as exc:
        raise ValueError(f"Unsupported Dorado layer tap name: {name!r}") from exc
    if layer_num < 1:
        raise ValueError(f"Dorado layer tap indices are 1-based, got {name!r}")
...
def load_dorado_encoder_state(
    *,
    model_path: str | Path,
    layers: Sequence[str] = _DEFAULT_DORADO_FEATURE_LAYERS,
) -> DoradoEncoderState:
    ...
    if str(scaling_cfg.get("strategy", "")).lower() != "pa":
        raise ValueError("Only Dorado models with scaling.strategy='pa' are supported.")
    std_cfg = dict(cfg.get("standardisation") or {})
    if not bool(std_cfg.get("standardise", 0)):
        raise ValueError("Dorado encoder state expects a model with enabled standardisation.")
    ...
    if transformer_depth <= 0:
        raise ValueError("Dorado encoder state expects a transformer encoder with positive depth.")
    if not transformer_layer_cfg:
        raise ValueError("Dorado encoder state expects transformer layer config in config.toml.")

    requested_layers = tuple(_normalize_layer_name(layer) for layer in layers)
    if not requested_layers:
        raise ValueError("Dorado encoder state requires at least one layer tap.")
    if len(set(requested_layers)) != len(requested_layers):
        raise ValueError(f"Duplicate Dorado layer taps are not allowed: {requested_layers}")
    ...
    valid_layers.add("crf_logits")
    unknown_layers = sorted(set(requested_layers) - valid_layers)
    if unknown_layers:
        raise ValueError(f"Unknown Dorado layer taps: {unknown_layers}")
    ...
    include_crf = "crf_probs" in requested_layers or "crf_logits" in requested_layers
    ...
    if max_conv_idx < 0:
        raise ValueError("Dorado encoder state requires at least one supported layer tap.")
    ...
    if include_crf:
        if not crf_cfg:
            raise ValueError("Dorado CRF extraction expects encoder.crf config in config.toml.")
...
def _apply_linear_upsample(x: jnp.ndarray, state: DoradoEncoderState) -> jnp.ndarray:
    if state.upsample_kernel is None or state.upsample_bias is None:
        raise ValueError("Dorado upsample weights are missing from the encoder state.")
...
def _apply_crf_encoder(x: jnp.ndarray, state: DoradoEncoderState) -> jnp.ndarray:
    if state.crf_kernel is None:
        raise ValueError("Dorado CRF weights are missing from the encoder state.")
    scores = x
...
def extract_dorado_features(x: jnp.ndarray, state: DoradoEncoderState) -> tuple[jnp.ndarray, ...]:
    ...
    if state.include_crf:
        ...
        if "crf_logits" in requested:
            feature_map["crf_logits"] = crf_scores
...
def extract_dorado_conv_features(x: jnp.ndarray, state: DoradoEncoderState) -> tuple[jnp.ndarray, ...]:
...
def extract_dorado_crf_logits(x: jnp.ndarray, state: DoradoEncoderState) -> jnp.ndarray:
    feature_map = dict(zip(state.layer_names, extract_dorado_features(x, state)))
    if "crf_logits" not in feature_map:
        raise ValueError("DoradoEncoderState must be loaded with layers including 'crf_logits'.")
    return feature_map["crf_logits"]
